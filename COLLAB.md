...
- [ ] **【部署任务】V148 ~ V164：作业管理系统 + 课后信息导入 + Bug修复 + 已入库记录浏览 + 任务记录导入按钮 + 提示词管理 + 学生姓名注入系统提示词 + 移除补充说明 + 迭代更新系统 + 导入AI处理修复 + 报错信息优化 + 作业管理UI优化 + 当前状态复制按钮 + API供应商预设 + 批量并发上限修复 + 实时进度显示修复**

  **分支：** `claude/auto-load-student-files-thGk8`
  **版本跨度：** V147 → V164（16个版本）
...
| V161 | 2026-02-11 | 学生当前状态一键复制按钮 | 待部署 |
| V162 | 2026-02-11 | API供应商预设系统 — 预配置供应商一键切换密钥和地址 | 待部署 |
| V164 | 2026-02-11 | 批量并发上限修复(40→100) + 测试本/课后信息提取实时字符数显示 | 待部署 |
