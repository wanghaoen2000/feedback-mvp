...
- [ ] **【部署任务】V192：前端 bundle 优化 — 懒加载 + 代码拆分**

  **分支：** `claude/review-version-191-issue-TxeMH`
  **版本跨度：** V191 → V192
  **新增依赖：** 无
  **数据库迁移：** 无

  **V192 变更：**
  - 4 个非首屏 Tab 组件改为 React.lazy() 懒加载（HomeworkManagement、LessonPrep、HomeworkCorrection、BatchProcess）
  - 切换 Tab 时按需加载，首屏只加载"课堂反馈"Tab 代码
  - vite.config.ts 新增 manualChunks 配置，将 React、Radix UI、tRPC/React Query 拆分为独立 chunk
  - 懒加载期间显示统一的加载占位（Loader2 旋转动画 + "加载中…"）
  - 纯前端优化，无功能变更

  **部署操作：**
  ```bash
  git fetch origin
  git merge origin/claude/review-version-191-issue-TxeMH   # 应直接 fast-forward
  npm run build
  webdev_save_checkpoint
  git push origin main
  ```
...
| V192 | 2026-02-21 | 前端bundle优化 — 4个Tab懒加载+manualChunks代码拆分 | 待部署 |
