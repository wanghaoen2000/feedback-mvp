...
- [x] 新增 parallelTasks 状态管理（每个任务独立状态）
- [x] 修改并行执行代码，每个任务开始/完成/失败时实时更新状态
- [x] 修改UI渲染，显示4个任务的独立进度
- [x] 完成的任务立即显示✅，不用等其他任务
- [x] 失败的任务显示❌和错误信息
- [x] pnpm build 无报错
...
## V63.11 (Step 3c): 小班课并行进度显示优化

- [x] 新增 classParallelTasks 状态管理
- [x] 修改并行执行代码，每个任务实时更新状态
- [x] 修改UI渲染，显示4个任务的独立进度
- [x] 气泡图任务下显示每个学生的状态
- [x] 一对一模式的进度显示不受影响
- [x] pnpm build 无报错
...
## V63.13 (Step 5): 小班课失败跳过功能

- [x] 排查一对一 skipStep 函数实现
- [x] 为小班课添加跳过功能（失败时显示跳过按钮）
- [x] 点击跳过后标记状态为"已跳过"
- [x] 一对一模式跳过功能不受影响
- [x] pnpm build 无报错
...
## V64.1 (Step 1): 后端新增单任务重做端点

- [x] 新增 POST /api/batch/retry-task 端点
...
- [x] 添加 batchParams 状态保存参数快照
- [x] 批量生成开始时保存参数（roadmap, templateType, storagePath, batchId）
- [x] 为成功/失败的任务添加重做按钮
...
## V64 Step 4: 清理调试日志 + 测试 + 发布

- [ ] 后端：删除所有 [SSE-DEBUG] 日志
- [ ] 前端：删除所有 [SSE-DEBUG] 日志
- [ ] 前端：心跳日志改为 console.debug 或删除
- [ ] 更新版本号为 V64
- [ ] pnpm build 无报错
- [ ] 测试场景1：4个任务，并发2，正常完成
- [ ] 测试场景2：2个任务，并发1，长时间任务
- [ ] git push 成功
- [ ] git tag v64 推送成功
