...
以下配置项存储在数据库 `system_config` 表中，通过网站"高级设置"界面配置，不需要写入 `.env` 文件或 Manus Secrets：
...
V66-V152 期间新增的功能（后台任务系统V112、作业管理V148等）没有新增环境变量。这些功能的配置全部通过数据库 `system_config` 表管理，包括：
